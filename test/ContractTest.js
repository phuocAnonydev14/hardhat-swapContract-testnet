...
describe("Lock", function () {
  ...
  async function deployContractAndSetVariables() {
    ...
    return {
      contract, owner, user1, infiToken, curveToken, zetToken, nativeTokenAddr,
    };
  }
  ...
  describe("Check set Rate function fail", async function () {
    ...
    it("msg.sender isn't owner", async function () {
      await expect(contract.connect(userAddress).setRate(infiTokenAddress, curveTokenAddress, rate)).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
  ...
  describe("Check swap token successfully", async function () {
    it("Swap token to token", async function () {
      ...
      beforeEach(async function () {
        ...
        await contract
          .connect(owner)
          .setRate(infiTokenAddress, curveTokenAddress, rateInfiTokenToCurveValue);
        await contract
          .connect(owner)
          .setRate(infiTokenAddress, nativeTokenAddr, rateCurveToInfiValue);
        //Add Pool
      ...
      const tokenOutAmount = Number((tokenInAmount * rateInfiTokenToCurveValue));
  ...
  describe("Check swap token function fail", async function () {
    ...
    it("Swap token to token", async function () {
      ...
      await expect(contract.connect(user1).swap(infiTokenAddress, curveTokenAddress, 0)).to.be.revertedWith("Amount must be greater than 0");
      await expect(contract.connect(user1).swap(infiTokenAddress, curveTokenAddress, toWei(depositAmount + 1))).to.be.revertedWith("ERC20: transfer amount exceeds balance");
    })

    // it("Swap token to native token", async function () {
    //   await expect(contract.connect(addr1).swap(infiTokenAddress, nativeTokenAddr, 0)).to.be.revertedWith("Amount must be greater than 0");
    //   await expect(contract.connect(addr1).swap(infiTokenAddress, nativeTokenAddr, toWei(depositAmount))).to.be.revertedWith("Failed to send Ether");
    // })
    //
    // it("Swap native token to token", async function () {
    //   await expect(contract.connect(addr1).swap(nativeTokenAddr, curveTokenAddress, 0, { value: 0 })).to.be.revertedWith("Amount must be greater than 0");
    //   await expect(contract.connect(addr1).swap(nativeTokenAddr, infiTokenAddress, toWei(10 * depositAmount), { value: toWei(10 * depositAmount) })).to.be.revertedWith("ERC20: transfer amount exceeds balance");
    // })
  })
});
